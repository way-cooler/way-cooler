...
mod test {
    ...
    fn button_object_test() {
        ...
        lua.globals().set("button0",
                Button::new(&lua, lua.create_table().unwrap()).unwrap())
           .unwrap();
        lua.globals().set("button1",
                Button::new(&lua, lua.create_table().unwrap()).unwrap())
           .unwrap();
        lua.eval(
            r#"
...
"#,
    ...
    fn button_property_test() {
        ...
        if let rlua::Value::UserData(any) = button_class.to_lua(&lua).unwrap() {
            count = any.get_user_value::<Table>().unwrap()
                       .get::<_, Table>("properties")
                       .unwrap()
                       .len()
                       .unwrap();
        }
