...
use wlroots::{self,
              ...
              wlroots_dehandle, Area, CompositorHandle, Cursor, CursorHandle, DragIconHandle,
              Origin, SeatHandle, SeatHandler, Size, SurfaceHandle, SurfaceHandler, XCursorManager};
...
impl Seat {
                        ...
                        edges: Edges)
    {
        ...
        self.action = Some(Action::Resizing { start: Origin { x: view_x, y: view_y },
                                              offset,
...
impl SeatHandler for SeatManager {
    ...
    fn cursor_set(&mut self,
                  ...
                  _: SeatHandle,
                  event: &SetCursorEvent)
    {
        if let Some(surface_handle) = event.surface() {
            use compositor_handle as compositor;
            use surface_handle as surface;
            let server: &mut ::Server = compositor.into();
            let ::Server { ref cursor_handle, ref mut seat, .. } = *server;
            use cursor_handle as cursor;
    ...
    {
        {
            ...
            seat.drag_icons.insert(DragIcon { handle: drag_icon_handle });
        }
