use wlroots::{
    pointer_events::*, wlroots_dehandle::wlroots_dehandle, Capability, CompositorHandle, PointerHandle,
    PointerHandler, WLR_BUTTON_RELEASED
};
...
impl PointerHandler for Pointer {
    ...
    #[wlroots_dehandle(compositor, cursor)]
    fn on_motion(&mut self, compositor_handle: CompositorHandle, _: PointerHandle, event: &MotionEvent) {
        use compositor_handle as compositor;
    ...
    #[wlroots_dehandle(compositor, cursor)]
    fn on_button(&mut self, compositor_handle: CompositorHandle, _: PointerHandle, event: &ButtonEvent) {
        use compositor_handle as compositor;
        let server: &mut ::Server = compositor.into();
        let ::Server {
            ref cursor_handle,
