...
use wlroots::{project_box, utils::current_time, wlroots_dehandle::wlroots_dehandle, Area,
              CompositorHandle, Origin, OutputHandle, OutputHandler, OutputLayoutHandle, Renderer,
              Size, SurfaceHandle};
...
impl OutputHandler for Output {
    ...
    fn on_frame(&mut self, compositor_handle: CompositorHandle, output_handle: OutputHandle) {
        ...
        let Server { ref mut layout_handle, ref mut views, .. } = *state;
                  ...
                  ly: i32)
{
    ...
    let (render_width, render_height) =
        (width * renderer.output.scale() as i32, height * renderer.output.scale() as i32);
                ...
                views: &mut Vec<Rc<::View>>)
{
    for view in views.iter_mut().rev() {
        ...
        view.for_each_surface(&mut |surface_handle: SurfaceHandle, sx, sy| {
                ...
                use surface_handle as surface;
                let (width, height) = surface.current_state().size();
                let (render_width, render_height) = (width * renderer.output.scale() as i32,
                ...
                if layout.intersects(renderer.output, render_box) {
                    let transform = renderer.output.get_transform().invert();
                    let matrix =
                        project_box(render_box, transform, 0.0, renderer.output.transform_matrix());
                    if !renderer.render_texture_with_matrix(surface.texture().as_ref().unwrap(),
                                                            matrix)
                    {
                        warn!("Could not render a surface");
                    }
