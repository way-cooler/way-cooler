...
pub use self::{
    config::{load_config, log_error, syntax_check, SyntaxCheckError},
    utils::*
};
...
thread_local! {
    ...
    /// Global Lua state.
    pub static LUA: RefCell<Lua> =
        RefCell::new(unsafe { Lua::new_with_debug() });
    ...
    /// Main GLib loop
    static MAIN_LOOP: RefCell<MainLoop> =
        RefCell::new(MainLoop::new(None, false));
}
...
pub fn run_awesome(lib_paths: &[&str], cmdline_path: Option<&str>) {
    LUA.with(|lua| {
        ...
        let regular_startup = lua.context(|ctx| {
            let (file_name, content) = load_config(ctx, cmdline_path)
                .map_err(|err| {
                warn!("Could not read init file: {}", err);
                SyntaxCheckError::IoError(err)
            })?;

            trace!("Read init file successfully");
            self::config::exec_config(ctx, &file_name, &content).map_err(|err| {
                log_error(err.clone());
        ...
        if regular_startup.is_err() {
            ...
            *lua = unsafe { Lua::new_with_debug() };
            ...
            lua.context(|ctx| {
                ...
                self::config::exec_config(ctx, "init.lua <DEFAULT>", self::config::DEFAULT_CONFIG)
...
fn emit_refresh(lua: rlua::Context) {
    if let Err(err) =
        signal::global_emit_signal(lua, ("refresh".to_owned(), Value::Nil))
    {
