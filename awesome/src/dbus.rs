...
fn process_request(bus_type: BusType, lua: &Lua, msg: &Message) -> rlua::Result<Option<Message>> {
    ...
    let (path, interface, member) = (path.unwrap_or_else(|| "".into()),
    ...
    let dbus_table = lua.globals().get::<_, Table>("dbus").expect("Could not get dbus table");
...
fn get_bus_by_name<'bus>(bus_name: &str) -> rlua::Result<&'bus LocalKey<GlobalConnection>> {
    match bus_name {
...
fn dbus_to_lua_value<'lua>(lua: &'lua Lua,
...
fn lua_value_to_dbus(lua: &Lua, type_: Value, value: Value) -> rlua::Result<MessageItem> {
    ...
    let is_ascii = type_.chars().next().map(|c| char::is_ascii(&c)).unwrap_or(false);
    if type_.len() > 1 || !is_ascii {
        return Err(RuntimeError(format!("{} is an invalid type name", type_)))
    }
    let type_ = ArgType::from_i32(type_.chars().next().unwrap() as i32).map_err(|_| {
                                                                           RuntimeError(format!(
    ...
    match (type_, value) {
        (ArgType::Array, Value::Table(value)) => {
            let size = value.len()?;
            if size % 2 != 0 {
                return Err(RuntimeError("your D-Bus signal handling method returned \
                                         wrong number of arguments"
                                                                   .into()))
            }
            ...
            MessageItem::new_array(list).map_err(|_| RuntimeError("Empty list is invalid".into()))
        }
        ...
        (type_, value) => {
            Err(RuntimeError(format!("Invalid type {:?} or value {:?}", type_, value)))
        }
    }
}
...
fn add_match(_: &Lua, (bus, name): (String, String)) -> rlua::Result<()> {
    ...
    bus.with(|bus| {
           let bus = bus.borrow();
           let bus = bus.as_ref().unwrap();
           bus.add_match(name.as_str()).map_err(|err| RuntimeError(format!("{}", err)))
       })?;
    Ok(())
}
...
fn remove_match(_: &Lua, (bus, name): (String, String)) -> rlua::Result<()> {
    ...
    bus.with(|bus| {
           let bus = bus.borrow();
           let bus = bus.as_ref().unwrap();
           bus.remove_match(name.as_str()).map_err(|err| RuntimeError(format!("{}", err)))
       })?;
    Ok(())
}

fn connect_signal<'lua>(lua: &'lua Lua,
                        (name, func): (String, rlua::Function))
                        -> rlua::Result<MultiValue<'lua>>
{
    let signals =
        lua.globals().get::<_, Table>("dbus").unwrap().get::<_, Table>(SIGNALS_NAME).unwrap();
    if signals.get(name.as_str())? {
        let error_msg = format!("Cannot add signal {} on D-Bus, already existing", name.as_str());
        warn!("{}", error_msg);
...
fn disconnect_signal(lua: &Lua, (name, _func): (String, rlua::Function)) -> rlua::Result<()> {
    let signals: Table = lua.globals().get::<_, Table>("dbus").unwrap().get(SIGNALS_NAME).unwrap();
    signal::disconnect_signals(lua, signals, name)
}

fn emit_signal<'lua>(lua: &'lua Lua,
                     (bus, path, interface, name, args): (String,
                      String,
                      String,
                      String,
                      MultiValue))
...
{
    if args.len() % 2 != 0 {
        let error_msg = "your D-Bus signal emitting metod has wrong number of arguments";
        warn!("{}", error_msg);
    ...
    bus.with::<_, rlua::Result<_>>(|bus| {
           let bus = bus.borrow();
           let bus = bus.as_ref().unwrap();
           let mut msg = Message::new_signal(path, interface, name).map_err(|_| {
                             RuntimeError("your D-Bus signal emitting \
                                           method had bad argument type"
                                                                        .into())
                         })?;
           msg.append_items(&args);
           bus.send(msg).map_err(|_| RuntimeError("Could not send D-Bus message".into()))
       })?;
    true.to_lua(lua)
}
