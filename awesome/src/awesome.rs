...
impl Default for AwesomeState {
    fn default() -> Self { AwesomeState { preferred_icon_size: 0 } }
}

impl Display for AwesomeState {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result { write!(f, "Awesome: {:p}", self) }
}
...
fn systray<'lua>(_: &'lua Lua, _: ()) -> rlua::Result<(u32, Value)> { Ok((0, Value::Nil)) }
...
fn load_image<'lua>(lua: &'lua Lua, file_path: String) -> rlua::Result<Value<'lua>> {
    let pixbuf =
        Pixbuf::new_from_file(file_path.as_str()).map_err(|err| {
                                                     rlua::Error::RuntimeError(format!("{}", err))
                                                 })?;
