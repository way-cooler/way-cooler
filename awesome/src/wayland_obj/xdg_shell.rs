...
impl XdgToplevel {
    fn new(
        ...
        wl_surface.commit();
        let cached_state = Box::new(XdgToplevelState {
            wl_surface,
            xdg_surface,
            buffer: None,
            size: Size::default()
...
fn create_xdg_surface(surface: &Proxy<WlSurface>) -> Result<Proxy<XdgSurface>, ()> {
    XDG_SHELL_CREATOR.with(|shell_creator| {
        ...
        shell_creator.get_xdg_surface(surface).map(|new_proxy| {
            new_proxy.implement(move |event, proxy: Proxy<XdgSurface>| {
...
fn unwrap_state_mut<'this>(
    proxy: &'this mut Proxy<xdg_toplevel::XdgToplevel>
