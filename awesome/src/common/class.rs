...
impl<'lua, S: ObjectStateType> From<AnyUserData<'lua>> for Class<'lua, S> {
    fn from(class: AnyUserData<'lua>) -> Self { Class { class, kind: PhantomData } }
}
...
impl<'lua, S: ObjectStateType> ClassBuilder<'lua, S> {
    ...
    pub fn build(self) -> rlua::Result<Class<'lua, S>> { Ok(self.class) }
}

impl<'lua, S: ObjectStateType> ToLua<'lua> for Class<'lua, S> {
    fn to_lua(self, lua: &'lua Lua) -> rlua::Result<Value<'lua>> { self.class.to_lua(lua) }
}

impl<S: ObjectStateType> Default for ClassState<S> {
    fn default() -> Self { ClassState { checker: Option::default(), instances: 0 } }
}
...
impl<'lua, S: ObjectStateType> Class<'lua, S> {
                   ...
                   -> rlua::Result<ClassBuilder<'lua, S>>
    {
        ...
        Ok(ClassBuilder { lua, class: Class { class: user_data, kind: PhantomData } })
    }

    pub fn checker(&self) -> rlua::Result<Option<Checker<S>>> {
        self.class.borrow::<ClassState<S>>().map(|class| class.checker.clone())
    }
}
...
{
    let class =
        lua.globals().get::<_, AnyUserData>(name).expect("Class was not set! Did you call init?");
